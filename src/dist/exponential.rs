...
impl Scorable<LogScore> for Exponential {
    ...
    fn diagonal_metric(&self) -> Array2<f64> {
        Array2::from_elem((self.scale.len(), 1), 1.0)
    }
    ...
    fn score(&self, y: &Array1<f64>) -> Array1<f64> {
        ...
        Zip::from(&mut scores)
            ...
            .for_each(|s, &y_i, &scale| {
                *s = scale.ln() + y_i / scale;
            });
        scores
    }

    fn d_score(&self, y: &Array1<f64>) -> Array2<f64> {
        // d/d(log_scale) of (ln(scale) + y/scale) = 1 - y/scale
        let n_obs = y.len();
...
impl Scorable<CRPScore> for Exponential {
    ...
    fn diagonal_metric(&self) -> Array2<f64> {
        ...
        ndarray::Zip::from(diag.column_mut(0))
            .and(&self.scale)
            .for_each(|d, &scale| {
                *d = 0.5 * scale;
            });
...
impl CensoredScorable<LogScoreCensored> for Exponential {
    ...
    fn censored_d_score(&self, y: &SurvivalData) -> Array2<f64> {
        ...
        for i in 0..n_obs {
            ...
            if e {
                // Uncensored: d/d(log_scale) of -log(pdf(t)) = d/d(log_scale)(ln(scale) + t/scale)
                // = 1 - t/scale
                d_params[[i, 0]] = 1.0 - t / self.scale[i];
            } else {
                // Censored: d/d(log_scale) of -log(S(t)) = d/d(log_scale)(t/scale)
                // = -t/scale (chain rule: d(t*exp(-log_scale))/d(log_scale) = -t/scale)
